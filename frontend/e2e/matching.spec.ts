...
const suggestions = {
  suggestions: [
    { escort_id: "escort-1", name: "Mei Ling", gender: "F", score: 3, flairs: ["Speaks Hokkien", "Wheelchair capable", "Gender preference met"] },
    { escort_id: "escort-2", name: "Siti Aishah", gender: "F", score: 1, flairs: ["Wheelchair capable"] },
  ],
  warning: null,
};

async function mockStandardApi(page: Page) {
  await page.route("**/matching-queue", (route) => route.fulfill({ json: [trip] }));
  await page.route("**/schedule", (route) => route.fulfill({ json: [{ trip_id: "trip-scheduled", elderly_name: "Mr Tan Ah Kow", escort_name: "Siti Aishah", appt_date: "2026-09-09", appt_time: "13:30:00", destination: "Ng Teng Fong General Hospital" }] }));
  await page.route("**/trips/trip-1/escort-suggestions?limit=3", (route) => route.fulfill({ json: suggestions }));
  await page.route("**/elderly-clients/elderly-1/matching-profile", async (route) => {
    const body = route.request().postDataJSON();
    await route.fulfill({ json: { elderly_id: "elderly-1", ...body } });
  });
  await page.route("**/trips/trip-1/confirm-escort", (route) => {
    const body = route.request().postDataJSON();
    return route.fulfill({ json: { trip_id: "trip-1", escort_id: body.escort_id, status: "scheduled", assignment_override: body.assignment_override } });
  });
}
...
test("confirms a suggested escort and removes the patient module", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: /Mdm Lim Siew Hoon/ })).toHaveCount(0);
});
...
test("edits matching details before confirming", async ({ page }) => {
  ...
  const patchRequest = page.waitForRequest("**/elderly-clients/elderly-1/matching-profile");
  await page.getByRole("button", { name: "Save and refresh" }).click();
  expect((await patchRequest).postDataJSON().dialect).toBe("Cantonese");
  await expect(page.getByText("Details saved. Suggestions have been refreshed.")).toBeVisible();
});
...
test("requires a reason for a manual override", async ({ page }) => {
  await page.route("**/matching-queue", (route) => route.fulfill({ json: [trip] }));
  await page.route("**/trips/trip-1/escort-suggestions?limit=3", (route) => route.fulfill({ json: { suggestions: [], warning: "No viable escort is available." } }));
  ...
  const confirm = page.getByRole("button", { name: "Confirm Siti Aishah with override" });
  await expect(confirm).toBeDisabled();
  await page.getByLabel("Reason for overriding").fill("Escort confirmed availability by phone.");
...
test("captures the patient modules and focused drawer at review widths", async ({ page }, testInfo) => {
  ...
  await page.screenshot({ path: "output/playwright/screenshots/matching-drawer-desktop.png", fullPage: true });

  await page.setViewportSize({ width: 1024, height: 768 });
  await page.screenshot({ path: "output/playwright/screenshots/matching-drawer-compact.png", fullPage: true });

  await page.setViewportSize({ width: 390, height: 844 });
  await page.screenshot({ path: "output/playwright/screenshots/matching-drawer-mobile.png", fullPage: true });
});
...
test("has no automatically detectable WCAG 2.2 A or AA violations", async ({ page }) => {
  ...
  await page.getByRole("button", { name: /Mdm Lim Siew Hoon/ }).click();
  await expect(page.getByText("Speaks Hokkien")).toBeVisible();
  const drawer = await new AxeBuilder({ page }).withTags(["wcag2a", "wcag2aa", "wcag21a", "wcag21aa", "wcag22aa"]).analyze();
